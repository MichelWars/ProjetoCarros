...
class CarsListView(ListView):   # herdando a classe listView
    ...
    def get_queryset(self):
        cars = (
            super().get_queryset().order_by('brand__name', 'model')
        )   # ordenação o __ serve para acessar o atributo de outra tabela
        search = self.request.GET.get('search')   # filtro
        if search:  # se houver busca
            cars = cars.filter(
                model__icontains=search
            )   # busca pelo valor do campo de busca
        return cars


@method_decorator(
    login_required(login_url='login'), name='dispatch'
)  # verifica se o usuario esta logado antes de permitir acesso a view
