...
def car_inventory_update():
    ...
    cars_value = (
        Car.objects.aggregate(  # realiza uma consulta no banco de dados
    ...
    CarInvetory.objects.create(  # cria um novo registro na model CarInvetory com os valores levantados na consulta acima
        cars_count=cars_count, cars_value=cars_value
    )


@receiver(
    post_save, sender=Car
)   # acompnhar evento de salva registro na model Car
def car_post_save(sender, instance, **kwargs):
    car_inventory_update()


@receiver(
    post_delete, sender=Car
)   # marca a função para acompnhar evento na model Car
